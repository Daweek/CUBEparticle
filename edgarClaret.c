    ...
    md_time0 = (time_v.tv_sec + time_v.tv_usec / 1000000.0);
      ...
      if(grape_flg == 1){
    	  ...
    	  for(i=0;i<NUMT;i++){
			  ...
			  stacksize = sizeof(thread_data_array);
			  ...
			  thread_data_array[i].range = range;
    	  ...
    	  for(i=0;i<n3;i+=3){
    		  aux 		=  (i)/(3*range);
    		  fc[i] 	= thread_data_array[aux].fc[i];
    		  fc[i+1] 	= thread_data_array[aux].fc[i+1];
    		  fc[i+2] 	= thread_data_array[aux].fc[i+2];
    	  }
